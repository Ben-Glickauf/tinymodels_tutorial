...
Instead of learning these kinds of hyperparameters during model training, we can *estimate* the best values for these values by training many models on resampled data sets and exploring how well all these models perform. This process is called **tuning**.
...
#library(vip)         # for variable importance plots (won't load)
```

## The cell image data, revisited

In our previous [*Evaluate your model with resampling*](https://www.tidymodels.org/start/resampling/) article, we introduced a data set of images of cells that were labeled by experts as well-segmented (`WS`) or poorly segmented (`PS`). We trained a [random forest model](https://www.tidymodels.org/start/resampling/#modeling) to predict which images are segmented well vs. poorly, so that a biologist could filter out poorly segmented cell images in their analysis. We used [resampling](https://www.tidymodels.org/start/resampling/#resampling) to estimate the performance of our model on this data.
...
Think of [`tune()`](https://hardhat.tidymodels.org/reference/tune.html) here as a placeholder. After the tuning process, we will select a single numeric value for each of these hyperparameters. For now, we specify our parsnip model object and identify the hyperparameters we will [`tune()`](https://hardhat.tidymodels.org/reference/tune.html).
...
Here we use a [`workflow()`](https://workflows.tidymodels.org/reference/workflow.html) with a straightforward formula; if this model required more involved data preprocessing, we could use [`add_recipe()`](https://workflows.tidymodels.org/reference/add_recipe.html) instead of [`add_formula()`](https://workflows.tidymodels.org/reference/add_formula.html).
...
The function [`collect_metrics()`](https://tune.tidymodels.org/reference/collect_predictions.html) gives us a tidy tibble with all the results. We had 25 candidate models and two metrics, `accuracy` and `roc_auc`, and we get a row for each `.metric` and model.
...
We can update (or “finalize”) our workflow object `tree_wf` with the values from [`select_best()`](https://tune.tidymodels.org/reference/show_best.html).
...
```{r eval = FALSE}
# doesn't run, wip packae won't load
#library(vip)
...
We can now tune our model, similar to what is shown in the previous article [Tune model parameters](https://www.tidymodels.org/start/tuning). Let's create a grid with 30 values for the hyperparameter we would like to tune:
